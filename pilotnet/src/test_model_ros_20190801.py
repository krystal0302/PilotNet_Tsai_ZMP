...
def road_segmention_img(img):
  ...
  red = detecte_red(blured)
  gray = cv2.cvtColor(red, cv2.COLOR_BGR2GRAY)
  (thresh, bw) = cv2.threshold(gray, 70, 255, cv2.THRESH_BINARY)
  kernel = np.ones((20, 20), np.uint8)
  closing = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, kernel)
  color = cv2.cvtColor(closing, cv2.COLOR_GRAY2RGB)
  color[:, :, 0] = color[:, :, 1] = 0
  result = color

  result = segment_img
  result[color[:, :, 2] > 200] = [0, 0, 255]
  #
  detected_white = detecte_white(blured)
  detected_white[detected_white > 0] = 255
  #
  detected_red = detecte_red(result)
  #
...
class image_converter:
  ...
  def __init__(self):
    ...
    model_name = 'model_save_function_python2_74.h5'
    print('---------------------------------------------------------------')
  ...
  def callback(self,data):
    ...
    with self.graph.as_default():
        ...
        steering_angel = speed_table[np.argmax(preds[0])]
